#include "dma_83c4_ex.h"
/*-----------------------------------*/
/**
 *
 */
/*-----------------------------------*/
int dmac_83c4_trans_mask_img_ch_fr(int _space_ch,int _space_frame,const int _sz)
{

	 if(dmac_83c4_pthread_mutex_lock()==SUCCESS){
					...
					if(dmac_83c4_pthread_mutex_unlock()==SUCCESS){
						return TRUE;
					}
...
void dmac_83c4_trans_mask_img_cmd_ctrl(CMD_CTRL*  _cmd)
{
		...
		void*  ps_virtual_addr_dst=dmac_83c4_get_mem_ps_virtual_base_addr();

		memcpy(ps_virtual_addr_dst,data_t,image_size);

		dmac_83c4_trans_mask_img_ch_fr(ch_space,ch_frame,image_size);
