 ...
 */
/*-----------------------------------*/
int AXILitemmapEx(int Addr,unsigned int *ptrUiFd,void **_pptr_virtual_addr,int MAP_SIZE)
{
	*ptrUiFd = open(MEM_DEV, O_RDWR|O_SYNC);
    if(ptrUiFd < 0)
    {
...
{
	mapObj->fd = open(MEM_DEV, O_RDWR|O_SYNC);
    if(mapObj->fd < 0)
    {
        PRINTF_DBG("open %s failed!\n",MEM_DEV);
        return -1;
